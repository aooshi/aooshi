...
namespace Aooshi.Images
{
    ...
    public class ImageMark
    {
        ...
        public string ImgMark
        {
            ...
            {
                _imgMark = value;
                if (!File.Exists(value)) throw new FileNotFoundException(string.Format("Not Find File '{0}';", value));
            }
        }
        ...
        {
            if (!savestream.CanWrite) throw new IOException("the stream can not be written.");
            ...
            try
            {
                using (Image image = Image.FromStream(stream))
                {
                    ...
                    if (!string.IsNullOrEmpty(_txtMark))
                    {
                        ...
                        SolidBrush b2 = new SolidBrush(Color.FromArgb(this._txtAlpha, this._txtcolor));
                    ...
                    else if (!string.IsNullOrEmpty(_imgMark))
                    {
                        ...
                        //加图片水印 
                        using (Image copyImage = Image.FromFile(_imgMark))
                        {
                    ...
                    {
                        throw new AooshiException("not set imgMark or txtMark");
                    }
