...
namespace Aooshi.Images
{
    ...
    /// 图片处理
    /// </summary>
    public class DisposeImage : IDisposable
    {
        MemoryStream stream;
        ...
        public DisposeImage(string path)
        {
        ...
        public DisposeImage(Stream stream)
        {
        ...
        public ImageFormat ImageFormat
        {
            ...
            set
            {
                ...
                using (Image image = Image.FromStream(this.stream))
                {
                    image.Save(ms, value);
                    image.Dispose();
                }
        ...
        /// <exception cref="AooshiException">Create Error</exception>
        public void CreateMark(ImageMark mark)
        {
            MemoryStream ms = new MemoryStream();
            if (!mark.Create(this.stream, ms)) throw new AooshiException("Create Image Mark Error " + mark.ErrorMessage);
            this.stream.Dispose();
            this.stream = ms;
        }
        ...
        {
            if (!mark.Create(this.stream, savestream)) throw new AooshiException("Create Image Mark Error " + mark.ErrorMessage);
        }
        ...
        public void CreateMark(ImageMark mark, string savepath)
        {
            ...
            {
                if (!mark.Create(this.stream, fs)) throw new AooshiException("Create Image Mark Error " + mark.ErrorMessage);
            }
        }


        /// <summary>
        /// 保存当前图片,使用此方法必需保证是使用的<see cref="DisposeImage"/> 重载方法
        /// </summary>
        public void Save()
        {
            if (string.IsNullOrEmpty(this.Path)) throw new AooshiException("Instead of using DisposeImage(string path) initialized");
        ...
        {
            if (!outputStream.CanWrite) throw new IOException("the stream can not be written.");
        ...
        {
            Image img = Image.FromStream(this.stream);
            img.Save(path, @if);
            img.Dispose();
        }
        ...
        public static ImageCodecInfo JpegCodeinfo
        {
            get
            {
                if (_JpegCodeinfo == null)
                {
                    ...
                    if (_JpegCodeinfo == null) throw new AooshiException("Not Find JPEG ImageCodecInfo");
                }
        ...
        public void SetQuality(int quality)
        {
            ...
            if (ici == null)
            {
                ici = Aooshi.Images.DisposeImage.JpegCodeinfo;
            }
